...
echo"<div class=\"alinha14\">FOTOS DA EMPRESA ($linhas)</div>";?> 
...
if(!empty($name) && in_array($type, $permitido)){
	...
	chmod ("../img2/fotos_empresa/$nome_imagem", 0777);	
	
$sql = "INSERT INTO fotos_empresa (`imagem` )
VALUES ('$nome_imagem'  )";
...
echo"<script language='javascript'>function alerta(){alert('Post cadastrado com sucesso!'); }alerta();document.location='javascript:history.go(-1)';</script>";
	...
	if ($type!=''){echo "<div id='notas'>Tipo de arquivo n�o aceito, favor enviar arquivos JPG</div>";}
...
<?php echo"<form name='upload' action='fotos-empresa' method='post' enctype='multipart/form-data'>"; ?>
...
<span class="tooltiptext">Selecione as imagens, limite m�ximo de 4MB todas imagens somadas!</span>
</div>
...
echo"<form name='excluir' method='POST' action='fotos-empresa?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='fotos-empresa?acao=remove&id=$id'><img src='../img2/fotos_empresa/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
if ($acao == 'remove'){
$sql = "DELETE FROM fotos_empresa WHERE id = '$id'  ";
$comando = mysqli_query($conn, $sql);;
...
{if($cod ==""){
echo "� necess�rio escolher alguma imagem!<br>"; echo "<a href='javascript: history.back();'>Voltar</a>";}

else {
$opcoes = $_POST['cod'];	
$opcoes_text = implode(", ", $opcoes);

$sql = "DELETE FROM fotos_empresa WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;

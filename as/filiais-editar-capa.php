...
if ($imagem ==''){echo"
<form action='filiais-editar-capa.php?acao=alterarx&id=$id' method='post'  enctype='multipart/form-data'>
...
<form action='filiais-editar-capa.php?acao=alterar1&id=$id' method='post'  enctype='multipart/form-data'>

<img src='../img2/filiais/$imagem' width='50%' border='0'> <br><br>
...
$msg = false;
  if(isset($_FILES['imagem'])){
    ...
    $diretorio = "../img2/filiais/"; //define o diretorio para onde enviaremos o arquivo
    move_uploaded_file($_FILES['imagem']['tmp_name'], $diretorio.$novo_nome); //efetua o upload
	chmod ("../img2/filiais/$novo_nome", 0777);
  
$sql="UPDATE  `filiais` SET  `imagem` =  '$novo_nome' WHERE  `id` ='$id'";
...
$caminho = "<script language='javascript'>
      ...
      $id = $_GET['id'];
      $sql="UPDATE  `filiais` SET `imagem` =  ''  WHERE  `id` ='$id';";

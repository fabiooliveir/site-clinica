...
<div class="alinha14"><a href="fotos">FOTOS >></a> FOTOS DA GALERIA</div></div></div>
...
if(!empty($name) && in_array($type, $permitido)){
	...
	chmod ("../img2/fotos_galeria/$nome_imagem", 0777);	
	
$sql ="INSERT INTO fotos_galeria (`imagem`, `postid`  )
...
<?php echo"<form name='upload' action='fotos-galeria' method='post' enctype='multipart/form-data'>"; ?>
...
$id_get = $_GET['id'];
$sql = "SELECT * FROM fotos_galeria WHERE postid = '$id_get'";
...
echo"<form name='excluir' method='POST' action='fotos-galeria?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='fotos-galeria?acao=remove&id=$id'><img src='../img2/fotos_galeria/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
$id = $_GET['id'];
if ($acao == 'remove'){
$sql = "DELETE FROM `fotos_galeria` WHERE id = $id";
$comando = mysqli_query($conn, $sql);;
...
$sql = "DELETE FROM fotos_galeria WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;

...
<div class="alinha14"><a href="exames">EXAMES</a> >> CADASTRAR ÍCONE</div></div></div>
...
VALUES (NULL ,  '$img' ,  'Exames' ,  '$post_id' ,  'Exames')";
...
<?php $post_id = $_GET['id']; echo"<form name='upload' action='imagem-png-exames?acao=cadastrar&id=$post_id' method='post' enctype='multipart/form-data'>"; ?>
...
<input type='hidden'  name='tipo' size='30' value='Exames' class='form'/>	
...
$id= $_GET['id'];
$sql = "SELECT * FROM imagem_png WHERE '$id' = postid and categoria = 'Exames'";
...
echo"<form name='excluir' method='POST' action='imagem-png-exames?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='imagem-png-exames?acao=remove&id=$id&postid=$postid'><img src='../img2/exames_png/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";

...
if ($imagem_depois ==''){echo"
<form action='antes-e-depois-capa-depois?acao=alterarx&id=$id' method='post'  enctype='multipart/form-data'>
...
<form action='antes-e-depois-capa-depois?acao=alterar1&id=$id' method='post'  enctype='multipart/form-data'>

<img src='../img2/antes_e_depois/$imagem_depois' width='50%' border='0'> <br><br>
...
$msg = false;
  if(isset($_FILES['imagem'])){
    ...
    $diretorio = "../img2/antes_e_depois/"; //define o diretorio para onde enviaremos o arquivo
    move_uploaded_file($_FILES['imagem']['tmp_name'], $diretorio.$novo_nome); //efetua o upload
	chmod ("../img2/antes_e_depois/$novo_nome", 0777);
  
$sql="UPDATE  `antes_e_depois` SET  `imagem_depois` =  '$novo_nome' WHERE  `id` ='$id'";
...
$caminho = "<script language='javascript'>
      ...
      $id = $_GET['id'];
      $sql="UPDATE  `antes_e_depois` SET `imagem_depois` =  ''  WHERE  `id` ='$id';";

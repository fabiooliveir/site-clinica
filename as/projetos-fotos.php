...
<div class="alinha14"><a href="projetos">PROJETOS</a> >> CADASTRAR FOTOS</div></div></div>
...
$sql = "INSERT INTO fotos_projetos (postid, `imagem` )
...
<?php echo"<form name='upload' action='projetos-fotos?id=$id' method='post' enctype='multipart/form-data'>"; ?>
...
<span class="tooltiptext">Selecione as imagens, limite máximo de 4MB todas imagens somadas! 1920px X 920px</span>
</div>
...
$id= $_GET['id'];
$sql = "SELECT * FROM fotos_projetos WHERE '$id' = postid";
...
echo"<form name='excluir' method='POST' action='projetos-fotos?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='projetos-fotos?acao=remove&id=$id&postid=$postid'><img src='../img2/fotos_projetos/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
$id = $_GET['id'];
if ($acao == 'remove'){
$sql = "DELETE FROM `fotos_projetos` WHERE id = $id";
$comando = mysqli_query($conn, $sql);;
...
$sql = "DELETE FROM fotos_projetos WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;


 echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!'); }alerta();document.location='projetos-fotos?id=$postid';
      </script>"; }}?>

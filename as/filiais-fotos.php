...
<div class="alinha14"><a href="filiais">FILIAIS</a> >> CADASTRAR FOTOS</div></div></div>
...
<div style="display: none"><a href="filiais-fotos"><div class="painel-banner-botoes1"><img src="img/categorias.jpg" width="100%"/></div></a></div>
...
echo"<a href=\"fotos-filial-sequencia?id=$id\"><div class=\"painel-banner-botoes2\"><img src=\"img/reorder.png\" width=\"100%\"/></div></a>";}?>
...
$sql = "INSERT INTO fotos_filiais (postid, `imagem` )
...
<?php echo"<form name='upload' action='filiais-fotos?id=$id' method='post' enctype='multipart/form-data'>"; ?>
...
$id= $_GET['id'];
$sql = "SELECT * FROM fotos_filiais WHERE '$id' = postid ORDER BY posicao ASC";
$comando = mysqli_query($conn, $sql);
while($res = mysqli_fetch_assoc($comando)){
...
echo"<form name='excluir' method='POST' action='filiais-fotos?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='filiais-fotos?acao=remove&id=$id&postid=$postid'><img src='../img2/fotos_filiais/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
$id = $_GET['id'];
if ($acao == 'remove'){
$sql = "DELETE FROM `fotos_filiais` WHERE id = $id";
$comando = mysqli_query($conn, $sql);;
...
$sql = "DELETE FROM fotos_filiais WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;


 echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!'); }alerta();document.location='filiais-fotos?id=$postid';
      </script>"; }}?>

...
<div class="alinha14"><a href="produtos">PRODUTOS</a> >> CADASTRAR FOTOS</div></div></div>
...
$cadastraImg = mysql_query("INSERT INTO fotos_produtos (postid, `imagem` )
...
<?php echo"<form name='upload' action='produtos-fotos?id=$id' method='post' enctype='multipart/form-data'>"; ?>
...
$id= $_GET['id'];
$query = "SELECT * FROM fotos_produtos WHERE '$id' = postid";
...
echo"<form name='excluir' method='POST' action='produtos-fotos?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='produtos-fotos?acao=remove&id=$id&postid=$postid'><img src='../img2/fotos_produtos/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
if ($acao == 'remove'){
$query = "DELETE FROM fotos_produtos WHERE id = '$id'  ";
$exe = mysql_query($query);

if ($exe){ echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!');}alerta();document.location='produtos-fotos?id=$postid';</script>";}}?>
...
$query = "DELETE FROM fotos_produtos WHERE id in (" . $opcoes_text . ")";
$exe = mysql_query($query);


 echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!'); }alerta();document.location='produtos-fotos?id=$postid';
      </script>"; }}?>

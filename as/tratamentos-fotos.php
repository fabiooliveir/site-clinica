...
<div class="alinha14"><a href="tratamentos">TRATAMENTOS</a> >> CADASTRAR FOTOS</div></div></div>
...
$sql = "INSERT INTO fotos_tratamentos (postid, `imagem` )
...
<?php echo"<form name='upload' action='tratamentos-fotos?id=$id' method='post' enctype='multipart/form-data'>"; ?>
...
$id= $_GET['id'];
$sql = "SELECT * FROM fotos_tratamentos WHERE '$id' = postid";
...
echo"<form name='excluir' method='POST' action='tratamentos-fotos?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='tratamentos-fotos?acao=remove&id=$id&postid=$postid'><img src='../img2/fotos_tratamentos/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
$id = $_GET['id'];
if ($acao == 'remove'){
$sql = "DELETE FROM `fotos_tratamentos` WHERE id = $id";
$comando = mysqli_query($conn, $sql);;
...
$sql = "DELETE FROM fotos_tratamentos WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;


 echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!'); }alerta();document.location='tratamentos-fotos?id=$postid';
      </script>"; }}?>

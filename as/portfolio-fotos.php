...
<div class="alinha14"><a href="portfolio">PORTFÓLIO</a> >> CADASTRAR FOTOS</div></div></div>
...
$sql = "INSERT INTO fotos_portfolio (postid, `imagem` )
...
<?php echo"<form name='upload' action='portfolio-fotos?id=$id' method='post' enctype='multipart/form-data'>"; ?>
...
$id= $_GET['id'];
$sql = "SELECT * FROM fotos_portfolio WHERE '$id' = postid";
...
echo"<form name='excluir' method='POST' action='portfolio-fotos?acao=remove2&id=$id&postid=$postid'>";


echo"
<div id=grid3><input type='checkbox' name='cod[]' value='$id'><br />
<a href='portfolio-fotos?acao=remove&id=$id&postid=$postid'><img src='../img2/fotos_portfolio/$imagem' title='Clique na imagem para excluir' border=0 width='100%' >
</a></div>";
...
$id = $_GET['id'];
if ($acao == 'remove'){
$sql = "DELETE FROM `fotos_portfolio` WHERE id = $id";
$comando = mysqli_query($conn, $sql);;
...
$sql = "DELETE FROM fotos_portfolio WHERE id in (" . $opcoes_text . ")";
$comando = mysqli_query($conn, $sql);;


 echo "<script language='javascript'>function alerta(){alert('Removido com sucesso!'); }alerta();document.location='portfolio-fotos?id=$postid';
      </script>"; }}?>
